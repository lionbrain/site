...
To connect an internet domain to an application running locally on port 9090 open a command terminal and run:

```bash
ssh -R 90:localhost:9090 localhost.run
```
  ...
  return (
    <>
      ...
      <pre><code parentName="pre" {...{
              "className": "bash"
            }}>{`ssh -R 90:localhost:${port} localhost.run
`}</code></pre>
    </>
  )
};
